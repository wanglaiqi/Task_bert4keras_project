...
# bert配置
bert_model_path = config_dict["bert_model_path"]
config_path = bert_model_path + 'bert_config.json'
checkpoint_path = bert_model_path + 'bert_model.ckpt'
dict_path = bert_model_path + 'vocab.txt'

# get the data_path
sogouqa_data_path = config_dict["data_path"]["SogouQA"]
webqa_data_path = config_dict["data_path"]["WebQA"]
random_order_path = config_dict["data_path"]["RandomOrder"]
# 标注数据
webqa_data = json.load(open(webqa_data_path + 'WebQA.json'))
sogou_data = json.load(open(sogouqa_data_path + 'SogouQA.json'))

# 保存一个随机序（供划分valid用）
if not os.path.exists(random_order_path + 'random_order.json'):
    random_order = list(range(len(sogou_data)))
    np.random.shuffle(random_order)
    json.dump(random_order, open(random_order_path + 'random_order.json', 'w'), indent=4)
else:
    random_order = json.load(open(random_order_path + 'random_order.json'))
